...
export async function redeemLink(_state: RedeemLinkState, formData: FormData): Promise<RedeemLinkState> {
  const session = await auth.api.getSession({ headers: await headers() })
  if (!session) return { done: true, error: 'سجّل الدخول أولًا لتفعيل الرابط' }
  ...
  if (!link || link.status !== 'active') {
    return { done: true, error: 'الرابط غير صالح أو سبق استخدامه' }
  }
  ...
  if (!result.ok) {
    const message =
      result.reason === 'used'
        ? 'تم استخدام هذا الرابط بالفعل — كل رابط يصلح لمرة واحدة فقط'
