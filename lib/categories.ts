...
import { categoryLabelAr, categoryLabelEn } from "./category-meta";
...
function buildCategories(): Category[] {
  ...
  return [...map.entries()]
    .map(([slug, { label, count }]) => ({
      ...
      labelEn: categoryLabelEn(slug, label),
      count,
    }))
...
export function navCategories(): { slug: string; labelAr: string; labelEn: string; count: number }[] {
  return CATEGORIES.map((c) => ({ slug: c.slug, labelAr: c.labelAr, labelEn: c.labelEn, count: c.count }));
}